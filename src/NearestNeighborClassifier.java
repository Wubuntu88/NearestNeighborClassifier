...
public class NearestNeighborClassifier {
	...
	private static final TreeSet<String> attributeTypes = new TreeSet<String>(
			(List<String>) Arrays.asList(BINARY, NOMINAL, ORDINAL, CONTINUOUS));
	...
	private void loadTrainingData(String fileName) {
		try {
			...
			this.numberOfAttributes = Integer
					.parseInt(componentsOfFirstLine[1]);
	...
	private ArrayList<Integer> classify(ArrayList<Record> testRecords) {
		ArrayList<Integer> listOfLabels = new ArrayList<>();// labels of the
															// classified
															// records
	...
	private double distance(Record record1, Record record2) {
		...
		for (int i = 0; i < record1.attrList.length; i++) {
			...
			case BINARY:// simple matching coefficient
				distances[i] = (int) record1.attrList[i] != (int) record2.attrList[i] ? 1
						: 0;
			case NOMINAL:
				distances[i] = (int) record1.attrList[i] != (int) record2.attrList[i] ? 1
						: 0;
			case ORDINAL:
				distances[i] = Math.abs(record1.attrList[i]
						- record2.attrList[i]);
			case CONTINUOUS:
				distances[i] = Math.abs(record1.attrList[i]
						- record2.attrList[i]);
